...
func InitializeGitHub(slack *Slack, a *config.AppConfig) GitHubApiHelper {
	...
	return GitHubApiHelper{Token: token, Slack: slack, AppConfig: a}
}
...
func (G *GitHubApiHelper) GetScaFindings(teamData map[string]models.Team, repos []models.IncludedRepo) AllVulnerabilitiesResponse {
	...
	for _, ir := range repos {
		G.AppConfig.Logger.Log("--" + ir.Org + "-" + ir.Name)
		hasNext = true
		currentPage := 1
		for hasNext {
			currentPage++
			...
			for _, n := range queryResponse.Data.Repository.VulnerabilityAlerts.Nodes {
				if queryResponse.Data.Repository.IsArchived || queryResponse.Data.Repository.IsDisabled {
					G.AppConfig.Logger.Log(fmt.Sprintf("-----WARNING----- Repository %s is archived or disabled", queryResponse.Data.Repository.Name))
					continue
				}
...
func (G *GitHubApiHelper) GetSastFindingsForRepo(org string, repo string, team string) []models.Vulnerability {
	...
	//G.AppConfig.Logger.Log(fmt.Sprintf("CheckMarx GET REQUEST response body: %s", string(body)))
	return vuls
}
