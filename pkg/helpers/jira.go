...
func (J *JiraApiHelper) CreateTickets(openVuls map[string]models.Vulnerability, teamData map[string]models.Team) []string {
	...
	for k, v := range vuls {
		...
		if v.HasPatch != "NO" {
			...
			J.AppConfig.Logger.Log(fmt.Sprintf("    ---New Vulnerability with NO FIX: %s", k))
		}
		...
		if sendToJira {
			...
			J.AppConfig.Logger.Log(string(responseData))
		} else {
			J.AppConfig.Logger.Log(fmt.Sprintf("  Tickets-Team: %s, createdAt: %s, Org: %s, repo: %s, package: %s,  Key: %s", v.Team, v.CreatedAt[0:10], v.Org, v.Repo, v.Package, k))
		}
...
func (J *JiraApiHelper) GetTicketsAlreadyInJira(teamData map[string]models.Team) map[string]bool {
	...
	for hasNextPage {
		...
		for _, i := range issueResponse.Issues {
			J.AppConfig.Logger.Log(fmt.Sprintf("    Ticket: %s, team: %s, VulnerabilityID:%s", i.Key, getTeamKeyFromJiraTeamID(i.Fields.Team.ID, teamData), i.Fields.VulnerabilityID))
	...
	J.AppConfig.Logger.Log(fmt.Sprintf("*************Length of TicketMap: %d", len(ticketMap)))
	return ticketMap
}
...
type Fields struct {
	...
	ApplicationName string      `json:"customfield_10680,omitempty"`
}
...
func (J *JiraApiHelper) GetSAUGJiraStatsForSlack(teamData map[string]models.Team, ticketsMovedToDoneByVulID map[string]bool) map[string]JiraStats {
	...
	for hasNextPage {
		...
		for _, i := range issueResponse.Issues {
			...
			J.AppConfig.Logger.Log(fmt.Sprintf("  Tickets Not Done Summary: %s, status: %s, priority: %s, req.date: %s", i.Fields.Summary, i.Fields.Status.Name, i.Fields.Priority.Name, i.Fields.RequestDueDate))
			...
			if requestedDueDateAsTime.Before(time.Now()) {
				J.AppConfig.Logger.Log(fmt.Sprintf("  PAST DUE - Summary: %s, status: %s, priority: %s, req.date: %s", i.Fields.Summary, i.Fields.Status.Name, i.Fields.Priority.Name, i.Fields.RequestDueDate))
				pd = append(jiraStatsResponse[teamKey].PastDue, i)
			}
			if requestedDueDateAsTime.After(time.Now()) {
				J.AppConfig.Logger.Log(fmt.Sprintf("  DUE IN FUTURE - Summary: %s, status: %s, priority: %s, req.date: %s", i.Fields.Summary, i.Fields.Status.Name, i.Fields.Priority.Name, i.Fields.RequestDueDate))
...
func (J *JiraApiHelper) GetSGRJiraStatsForSlack(teamData map[string]models.Team) map[string]JiraStats {
	...
	for hasNextPage {
		...
		for _, i := range issueResponse.Issues {
			...
			J.AppConfig.Logger.Log(fmt.Sprintf("  SGR Tickets Not Done Summary: %s, status: %s, priority: %s, req.date: %s", i.Fields.Summary, i.Fields.Status.Name, i.Fields.Priority.Name, i.Fields.RequestDueDate))
			requestedDueDateAsTime, _ := time.Parse("2006-01-02", i.Fields.RequestDueDate)
	...
	for hasNextPage {
		...
		for _, i := range issueResponse.Issues {
			...
			if !openMap[id] && !openMap[id2] && !strings.Contains(i.Fields.Summary, TICKET_NO_FIX_SUMMARY_SUFFIX) {
				...
				J.AppConfig.Logger.Log(fmt.Sprintf("  Ticket-Key %s, ID: %s, status: %s, team: %s", i.Key, i.Fields.VulnerabilityID, i.Fields.Status.Name, team))
...
func (J *JiraApiHelper) TransitionTicketsToDone(tickets map[string][]TicketsTobBeUpdatedOrTransitioned, teamData map[string]models.Team) {
	...
	for team, tMap := range tickets {
		for _, t := range tMap {
			J.AppConfig.Logger.Log(fmt.Sprintf("   Tickets -- key: %s, status: %s, team: %s", t.IssueKey, t.Status, team))
			if !teamData[team].JiraEnabled {
				continue
			}
			J.AppConfig.Logger.Log(fmt.Sprintf("      Moving Ticket to Done -- key: %s", t.IssueKey))
			switch t.Status {
			case "To Do":
...
func (J *JiraApiHelper) GetTicketsThatNowHaveAFix(currentVuls map[string]models.Vulnerability, resolvedIds map[string]bool) []TicketsTobBeUpdatedOrTransitioned {
	...
	for _, i := range issueResponse.Issues {
		...
		if strings.Contains(i.Fields.Summary, TICKET_NO_FIX_SUMMARY_SUFFIX) && currentVuls[vulID].HasPatch != "NO" && !resolvedIds[vulID] {
			J.AppConfig.Logger.Log(fmt.Sprintf("  Tickets to Update with Patch-- %s, Summary: %s, New Patch: %s, vulID: %s", i.Key, i.Fields.Summary, currentVuls[vulID].HasPatch, vulID))
...
func (J *JiraApiHelper) UpdateIssuesThatNowHaveAFix(tickets []TicketsTobBeUpdatedOrTransitioned, teamData map[string]models.Team) {
	...
	for _, t := range tickets {
		...
		J.AppConfig.Logger.Log(fmt.Sprintf("  Ticket to be Updated with New Patch-- %s, Summary: %s, Team: %s, New Patch: %s", t.IssueKey, t.Summary, t.Team, t.NewPatch))

		if !teamData[t.Team].JiraEnabled {
			continue
		}
		J.AppConfig.Logger.Log(fmt.Sprintf("  ----Ticket being Updated with New Patch-- %s, Summary: %s, Team: %s, New Patch: %s", t.IssueKey, t.Summary, t.Team, t.NewPatch))
...
func (J *JiraApiHelper) MoveNewTicketsWithNoFixToBlocked(tickets []string) {
	...
	for _, t := range tickets {
		J.AppConfig.Logger.Log(fmt.Sprintf("   Ticket: %s", t))
...
func (J *JiraApiHelper) MoveTicketsFromBlockedToToDo(tickets []TicketsTobBeUpdatedOrTransitioned, teamData map[string]models.Team) {
	...
	for _, t := range tickets {
		J.AppConfig.Logger.Log(fmt.Sprintf("   Ticket: %s, Team: %s", t.IssueKey, t.Team))
		if teamData[t.Team].JiraEnabled {
...
func (J *JiraApiHelper) JiraGetRequest(url string) IssueResponse {
	...
	//J.AppConfig.Logger.Log(fmt.Sprintf("JIRA GET REQUEST response body: %s", string(body)))
