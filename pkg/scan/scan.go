...
func StartScan(a *config.AppConfig) {
	...
	slack := helpers.InitializeSlack(a)
	jiraApiHelper := helpers.InitializeJIRA(slack, a)
	googHelper := helpers.InitializeGoogle(slack, a)
	//msTeams := helpers.InitializeTeams(a)
	...
	var gitHubAllVulnerabilities = helpers.AllVulnerabilitiesResponse{}
	...
	var gitHubProductionVulnerabilities = helpers.VulnerabilityCategories{}
	var gitHubDevelopmentVulnerabilities = helpers.VulnerabilityCategories{}

	if scanOptions.GitHubEnabled {
		gitHubApiHelper := helpers.InitializeGitHub(slack, a)
	...
	// Send Slack Messages
	slack.SendTeamSlackMessages(configData.Teams, repoReportSortedArr, lastRunDate, jiraStatsSaug, jiraStatsSGR, ticketsToBeTransitionedToDone.TicketsByTeam)
...
func getConfigData(isProd bool, s *helpers.Slack) models.ConfigData {
