...
func InitializeSlack(logger Logger) *Slack {
...
func (S *Slack) SendSlackErrorMessage(message string, channelID string) error {
	...
	S.Logger.Log(fmt.Sprintf("  Error/Status Message sent successfully to %s channel at %s", ChannelID, timestamp))
	return nil
}
...
func (S *Slack) SendSlackMessage(message string, channelID string, team string) error {
	...
	if err != nil {
		...
		S.Logger.Log(fmt.Sprintf("  Message sent successfully to team: %s channel: %s, at %s", team, ChannelID, timestamp))
	}
	return nil
}

func (S *Slack) SendTeamSlackMessages(teamData map[string]models.Team, repoReport []models.RepoSummaryForReport, lastRunDate string, jiraStatsSAUG map[string]JiraStats, jiraStatsSGR map[string]JiraStats, ticketsMovedToDone map[string][]TicketsTobBeUpdatedOrTransitioned, forceSlack bool) {

	S.Logger.Log("\n---Sending Slack Messages---\n")
	slackData := convertReportDataForSlack(repoReport)

	slackMessage := ""

	for team := range teamData {
		slackChannel := SLACK_CHANNEL_CARL_TEST_ID
		// if teamData[team].SlackEnabled && ((time.Now().Weekday().String() == "Monday" || time.Now().Weekday().String() == "Thursday") || forceSlack) {
...
func convertReportDataForSlack(repoReport []models.RepoSummaryForReport) map[string][]slackRepoData {
