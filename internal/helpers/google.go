...
func InitializeGoogle(s *Slack, logger Logger) GoogleHelper {
	...
	return GoogleHelper{DriveSrv: srv, Slack: s, Logger: logger}
}
...
func (G *GoogleHelper) UpdateDriveFile(dfInfo DriveFileInfo, contents string) {
	...
	G.Logger.Log(fmt.Sprintf("Drive file %s updated", dfInfo.Name))
}
...
func (G *GoogleHelper) GetDriveFileInfo() map[string]DriveFileInfo {
	...
	if len(r.Files) == 0 {
		G.Logger.Log("No Google Drive files found.")
	} else {
